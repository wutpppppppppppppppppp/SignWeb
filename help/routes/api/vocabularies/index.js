...
async function vocabulariesRoutes(fastify) {
  fastify.get("/", opt, async function (request, reply) {
    try {
      ...
      if (vocabularies.length === 0) {
        ...
        reply.code(404).send({ error: "No vocabularies found for this name" });
      } else {
        fastify.log.info(
  ...
  fastify.get("/search", opt, async function (request, reply) {
    try {
      const { name } = request.query;
      const vocabulariesCollection = fastify.mongo.client
        .db("sample_sign")
